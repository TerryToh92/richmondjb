...
export default function WhatsAppButton({
  ...
  return (
    <a
      ...
      onClick={() => track("whatsapp_click", { message })}
      className={`inline-flex items-center justify-center gap-2.5 rounded-sm bg-fire px-7 py-3.5 text-[15px] font-semibold text-bg transition hover:bg-fire-2 ${className}`}
    >
...
/** 品牌香槟金主 CTA（非 WhatsApp 的强调按钮）——浅金底配深字 */
export function FireButton({
  ...
  return (
    <a
      href={href}
      className={`inline-flex items-center justify-center gap-2 rounded-sm bg-fire px-7 py-3.5 text-[15px] font-semibold text-bg transition hover:bg-fire-2 ${className}`}
    >
      {children}
    </a>
  );
